...
f = f.assign(d1=(f.c1 == (f.b1 == f.r1)))
f = f.assign(c2=((f.r1 & ~f.b1) | (f.c1 & ~f.b1) | (f.r1 & f.c1)))
f = f.assign(C3=[0, 0, 0, 0, 1, 1, 1, 0])  # player

f = f.sort_values(by=['c1', 'r1', 'b1'], ascending=False, ignore_index=True).astype(int)
assert (hex(np.dot(f.C3, 2 ** f.index)) == "0x0E")

f = f.assign(not_c1=(1 - f.c1))
f = f.assign(not_r1=(1 - f.r1))
f = f.assign(not_b1=(1 - f.b1))

f = f.assign(not_d1=(1 - f.d1))
f = f.assign(not_c2=(1 - f.c2))

for ii in permutations(['c1', 'r1', 'b1']):
