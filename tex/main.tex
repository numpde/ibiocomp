...
\subsection{The game of 15 sticks%
	\texorpdfstring{\footnote{%
		...
		%
		See the link for the codes
		and the PDF file with sharp figures.
		%
		Thanks to:
		Y.~Benenson and M.~Chen.
	}}{}%
}
...
\subsection{Insulation of genetic context and RBS} \label{ss:insulation_RBS}
...
%
Insulating sequences can also be placed
upstream of promoters (including \promoter{BAD})
...
(Sal, DAPG: $\approx$12h;
3OC6, 3OC12, pC, MMF: $\approx$3h;
IV, NG: no delay).
%
%
See \cite[SM]{DuETAL2020}
...
%was shown by \citet{YanETAL2017} to repress phlG.

%

We use PhlF as the transcription factor repressed by DAPG.
%
\citet[Fig.~2a]{DuETAL2020}
use the gene cluster PhlABCD
for the synthesis of DAPG.
...
%
The dimer 
binds upstream of the promoter {hrpL} 
and opens the $\sigma^{54}${-}RNAP{-}hrpL transcription complex.
%
%
...
2)~%
two short RNAs
that inhibit the output mRNA
of the other two inputs
using 
the ``three-way junction'' mechanism of 
...
\item
    ...
    to sense
    the signals \ce{s_1} (pC-HSL), \ce{r_1} (IV-HSL) and \ce{c_1} (Sal).
\item
    ...
    %
    two kinds of inhibitory trigger RNAs (\ce{bd} and \ce{cd}),
    one ``co-trigger'' RNA \ce{mk}
    and an output mRNA. 
    %
    %
    The trigger RNA \ce{bd} / \ce{cd}
...
\item
    ...
    the exposed sequence \ce{mn} pries open the RBS,
...
%
%
In the figures we refer to ``a.u.'' 
instead of seconds,
because these choices lead to timescales
that are too fast to be realistic
(perhaps by a factor of 10).


\subsection{Player A} \label{s:sim:player}


We implemented
the response \ce{r_0} and \ce{r_1} of Player A 
in SymBiology
as sketched out in \S\ref{ss:cello}.
%
The steady-state response is shown in Fig.~\ref{f:player_response}.
...
\subsubsection*{Carry flag \texorpdfstring{\ce{c_2}}{c2}} \label{s:sim:bit1:c2}


We implemented \ce{c_2}
in SymBiology
as indicated in \S\ref{ss:cello}.
%
Recall that \ce{c_2} is emitted immediately,
bypassing the memory unit.
%
Fig.~\ref{f:sub_response} shows the steady-state response.
%
%We used cross-inhibition from \S\ref{ss:xinh}
%as the last gate.


\subsubsection*{Difference \texorpdfstring{\ce{d_1}}{d1}} \label{s:sim:bit1:d1}


We implemented \ce{d_1}
in SymBiology
as in \S\ref{ss:3xor}
with 
the buffer/shelving mechanism (\S\ref{ss:shelf})
and
the recombinase memory unit (\S\ref{ss:memory}).
%
%
...
\begin{figure}[!p]
    \centering
    % don't add path: it's set globally
    \includegraphics[height=(\textheight-3\baselineskip)]{xor_ribocomputing/xor_circuit_plot.pdf}
    \caption{%
        Genetic circuits for the 3-\XOR DNF \eqref{e:3xor-dnf}
        from \S\ref{ss:3xor}.
        Cf.~Fig.~\ref{f:logic_3xor}.
        %
        Ribozyme insulators are placed 
        just upstream of the switch RBS
        for non-constitutive promoters,
        see \S\ref{ss:insulation_RBS}.
    }
...
\begin{figure}[!p]
    ...
    \caption{%
        ...
        of the genetic implementation
        of the 3-\XOR DNF \eqref{e:3xor-dnf}
        from \S\ref{ss:3xor}.
        %
        To avoid clutter
        the wiring is indicated by the arrows.
        %
        \ce{Y} indicates active output mRNA translation.
        %
        Cf.~Fig.~\ref{f:p_construct_xor}.
    }
...
is switched temporarily by \ce{Xis};
the bistable relay is switched stably by \ce{Int}
either up (from above) or down (from below),
but remains unchanged without \ce{Int}.
%
%
The 3-\XOR gate
