...
phenol 
found in some strains of the Gram-negative
\emph{P.~fluorescens}%
\starlink{https://en.wikipedia.org/wiki/Pseudomonas_fluorescens}
\cite{SchniderkeelETAL2000}.
}
%
\hh{
...
\TODO{\hh{it seems this switch-OFF thing is not used in the enhanced AND gate? Should we put it in the XOR?} \ra{moved here:}}
...
The circuit consists of 3 interfering RNAs with their own promoter and terminator, and an output. The trigger RNA \ce{bd} switches off the IV-HSL triggered output and the trigger RNA \ce{cd} switches off the Sal triggered output. The switch OFF RBS follows the design in Fig.~\ref{f:switch_off_RBS}. 
