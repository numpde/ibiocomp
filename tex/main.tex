...
\begin{figure}
    \centering
    \includegraphics[width = \linewidth]{xor_ribocomputing/logic_gate_3xor.pdf}
    \caption{
    \hh{
    Logic gates for the 3-\XOR . Signal conduction downstream of pC-HSL is highlighed in blue for illustration. The \NOT gate is constructed by producing trigger RNA that switch OFF the RBS. The co-trigger RNA \AND gate is constructed by the timer that switch ON the RBS. See \S\ref{ss:3xor} and Fig.\ref{f:switch_RBS}.
    }
    }
...
of the memory unit with basic gates.
%
The right part (red) is closer
to our genetic implementation using
