...
\TODO{Describe the final variant}

\hh{We propose a 3-input-xor-gate constructed combined switch ON/OFF ribocomputing designs. For each input signal, it activates the output on its own responsive circuit while producing two trigger RNAs (repressor) that switches OFF the output of the other two. The switch OFF mechanism is based on three-way junction (3WJ) repressors proposed in \cite{Kim2019}).
A third trigger RNA (activator) is produced by each input signal; and when all 3 signals are present, three activator trigger RNAs form a trimer structure that activates the output on the fourth genetic circuit. The 3-input-AND-gate is based on the ribocomputing proposed in \cite{GreenETAL2017}.}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/xor_ribocomputing/switch_off.pdf}
    \caption{Switch OFF RBS. The unstable hairpin structure is sealed when the trigger RNA is present, thus repressing the gene expression.}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/xor_ribocomputing/switch_on.pdf}
    \caption{Switch ON RBS. The stem of the stable hairpin structure will be opened when the trigger RNA is present, thus exposing the RBS and activate the gene expression}
...
\begin{figure}
    ...
    \caption{3 trigger RNAs AND gate. When all 3 trigger RNAs are present, a trimer is formed, which then switch ON the RBS.}
