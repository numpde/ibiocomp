...
\cite[\href{https://www.nature.com/articles/s41467-020-17993-w/figures/3}{Fig.~3c}]{DuETAL2020}.
...
\TODO{\ra{I don't see how this is supposed to work:}}
...
\item
    ...
    (with \ce{c_1}).
    %
    \hh{Now the medium from Bit~0 contains updated \ce{s_0}.}
\item
    Admix the media of Player~A and Bit~0
    (with \ce{r_0}/\ce{r_1}/\ce{s_0}/\ce{s_1}/\ce{c_1})
    to Bit~1, wait for response
    (with \ce{c_2}). \hh{Now the medium from Bit~1 contains updated \ce{s_0} and \ce{s_1}.}
\item
    \hh{Half of the medium with updated \ce{s_0} and \ce{s_1} is transferred to the tube containing Players A and B. \ce{w_B} is added so that Player B is now in the game.}
    
    \hh{The medium from Bit~0 and Bit~1 computation (containing \ce{c_2} is now added to Bit~2 and wait for the response \ce{c_3}.}
    
    \hh{In the meanwhile, the medium for Bit~1 and Bit~0 is washed.}
    For the following we need only \ce{c_2}.
    %
    Thus we clean the medium of Bit~1,
    and add \ce{w_A} again.
    %
    Now, \ce{c_2} is gone and also the input to Bit~1
    is gone.
    %
    \TODO{How do we get \ce{c_2}?} 
    
\item
    \hh{Admix the medium from Bit~3 to Bit~4}
\end{enumerate}
...
\TODO{what is the url of the circuit figure, if any; why is it upside-down}
...
\TODO{we don't do PNG b/c that doesn't scale well (neither up nor down). unfortunately, www.circuit-diagram.org doesn't export eps or pdf, but it does export svg, from which I make pdf}
...
\TODO{$c1$ should be projected immediately}
