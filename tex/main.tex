...
\TODO{what is leader and what is scar}
...
\TODO{simplify, keep what's relevant:}
...
which is referred to as toehold swithes 
\cite{Green2014}.
}
...
It is suggested 
\cite{Ramos2005}
\TODO{where? cldn't find this claim \url{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1197418/pdf/0030-04.pdf}}
...
\hh{
2,4-diacetylphloroglucinol (DAPG) is a phenol found in some Gram-negatives 
\emph{P.~fluorescens}%
...
%
\hh{
\citet{SchniderKeel2000} suggested that DAPG is involved 
\TODO{``involved''?}
in phl gene clusters. 
}
%
\ra{
The Tet \TODO{TetR?} repressor homolog phlF binds its operator sequence (phlO)
...
works  \TODO{don't understand ``works''}
...
Methylenomycin is an antibioitic \TODO{a what} produced 
by \emph{S.~coelicolor}%
\starlink{https://en.wikipedia.org/wiki/Streptomyces_coelicolor}%
.
%
\hh{
It's synthesis involves \TODO{``involves''?} methylenomycin furans (Mmf). 
}
%
\ra{
...
but
accumulation of \signal{Mmf} releases \protein{MmfR} \cite{ORourkeETAL2009}. 
}
%
The TetR homolog MmfR is a homodimer \cite{Zhou2020}, 
\TODO{is a dimer or forms a dimer?}
which contains two Mmf recognition sites.
%
Different R-groups \TODO{whats the connection} may alter the binding affinity \cite[p.2-3]{Zhou2020}. 
%
\citet{DuETAL2020} use an MMF with a branched side chain.
\ra{they do?} \hh{p.2 MMF has the chemical structure} \TODO{exact ref}
...
We refer to it 
\ra{what is ``it''}
as switch-ON RBS regulation 
(appendix \TODO{ref}, \ra{Fig.~}\ref{f:switch_on_RBS}).
}
%
%
%
...
%
\TODO{fix text}
However, when a trigger RNA ab that can "sealed" the hairpin, the translation is then blocked. 
We refer to it as switch-OFF RBS regulation (appendix, figure \ref{f:switch_off_RBS})
}
%
%
...
\hh{
In figure \ref{f:p_construct_xor}, the TF \TODO{TF?} circuit provides the transcription factors required to sense the QS signals \ce{s_1} pC-HSL, \ce{r_1} IV-HSL and \ce{c_1} Sal.
...
\begin{figure}
    ...
    \TODO{these are facsimiles; provide \emph{specific} reference for each, incl figure number}
    \TODO{include backref in caption}
\end{figure}
...
\begin{figure}
    ...
    \TODO{facsimile; provide \emph{specific} reference, incl figure number}
    \TODO{include backref in caption}
\end{figure}
    
\begin{figure}
    \centering
    \includegraphics[height=0.95\textheight]{xor_ribocomputing/plot-3.pdf}
    \caption{Genetic circuits for the 3-input \XOR gate.}
    \label{f:p_construct_xor}
    \TODO{backref in caption}
\end{figure}
