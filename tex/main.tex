...
\usepackage{graphicx}
\graphicspath{{../images/}, {../code/20201231_SymBio_All}}
...
%
In fact, since the bit $s_3$ is otherwise unused
no dedicated channel is necessary for it
(YFP is expressed instead).
%
Thus we need \emph{eight} orthogonal intercellular channels.
%
This is possible according to \cite{DuETAL2020},
where 10 de novo intercellular signaling pathways
were designed
and
...
we avoid putting LuxR and RpaR sensors
in one cell (cf.~Table~\ref{t:signals}).
...
is present in the medium in sufficient quantity,
...
we found the three-input one-output gates
%
\begin{itemize}
\item
    $
...
%
We implemented those with some modifications.
%
\TODO{explain}
...
%
This achieves the temporal separation 
discussed in \S\ref{s:general} and Fig.~\ref{f:logical-subtractor01}.


\subsection{Heterodimeric transcription factors}

\subsubsection*{HrpRS} \label{ss:HrpRS}

\ra{This is useful as an \AND gate}
...
\ra{Probably not necessary here; just in the XOR section}

RNA-based circuits
were proposed/evaluated in \emph{E.~coli} in \cite{GreenETAL2017}.
%
...
and 
can be written 
in \href{https://en.wikipedia.org/wiki/Disjunctive_normal_form}{DNF}
as 
$
...
\vspace{0pt}
\includegraphics[width=\linewidth]{code/20201231_SymBio_All/Bit1/response_d1_tempo.pdf}
\end{minipage}
%
...
\begin{tabular}{cccc}
    & \ce{s_1} & \ce{r_1} & \ce{c_1} \\
    \ce{B_1} & 10 & 10 & 10 \\
    \ce{A_1} & {\color{gray}$0.01$} & {\color{gray}$0.01$} & {\color{gray}$0.01$} \\
    \ce{B_2} & 10 & 10 & {\color{gray}$0.01$} \\
    \ce{A_2} & {\color{gray}$0.01$} & {\color{gray}$0.01$} & {\color{gray}$0.01$} \\
\end{tabular}
}
\end{minipage}

\caption{%
Subtractor A, bit 1/\ce{d_1},
across the phases \ce{B_1}-\ce{A_1}-\ce{B_2}-\ce{A_2}
(with interphases).
%
The table shows the inputs.
%
As required,
we have
$\ce{d_1} \approx 10$ 
