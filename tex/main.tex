...
\begin{figure}
	\centering
	\includegraphics[width=0.3\linewidth]{circuits/Logical-PlayerA}
	\caption{%
		Logical circuit for Player A.
		%
		Input: 
		wake up flag \cee{w_A}
		and the lowest bits
		\cee{s_1} and \cee{s_0}
		from the current number of sticks.
		%
		Output:
		number of sticks that 
		the player chooses to take
		encoded as \cee{r_0 + 2 r_1}.
		%
		Note that the output is cut off
		when \cee{w_A} is absent.
	}	
	\label{f:logical-playera}
\end{figure}
